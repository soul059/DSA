...
// linked list to remove duplicates it must be sorted
Node<int>* removeDuplicates(DoublyLinkedList<int>& list) {
    if (!list.getHead()) return nullptr;
    ...
    while (current) {
        ...
        if (nextNode) {
            ...
            list.setTail(current); // Update tail if we reached the end
        }
        current = nextNode;
    }
    return list.getHead();
...
int main(){
    ...
    cout << "Original List: ";
    list.display();
    Node<int>* newHead = removeDuplicates(list);
